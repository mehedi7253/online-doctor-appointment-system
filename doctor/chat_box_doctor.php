                                        ...
                                        <p>
                                            <?php
                                                if (isset($_GET['message_id'])){
                                                    ...
                                                    $get_type_id = "SELECT * FROM doctors WHERE doctor_id = $message_id";
                                                    $get_res     = mysqli_query($connect, $get_type_id);

                                                    $data = mysqli_fetch_assoc($get_res);

                                                    $type = $_SESSION['doctor'];

                                                }

                                                $id = $userdata['doctor_id'];

//                                                $view_message = "SELECT * FROM chat WHERE msg_id = $id ORDER BY date_time ASC";
                                                  $view_message =  "SELECT * FROM chat WHERE msg_id = '$userdata[doctor_id]' AND to_id = '$message_id'";
                            ...
                            <div class="card-footer">
                                ...
                                if (isset($_POST['btn'])){
                                    ...
                                    $sql_msg = "INSERT INTO chat (message, msg_id, m_name, image, date_time, to_id, type) VALUES ('$message', '$msg_id', '$m_name', '$image', '$create', '$to_id', '$type')";
                                    $res_msg = mysqli_query($connect, $sql_msg);

                                    echo "<script>document.location.href='chat_box_doctor.php?message_id=$message_id'</script>";
...
<!--                                        <input type="text" name="tpe"   value="--><?php //echo $data['tpe'];?><!--" class="form-control">-->
                                        <input type="number" name="msg_id" hidden value="<?php echo $userdata['patient_id'];?>" class="form-control">
