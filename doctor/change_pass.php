                               ...
                               <?php
                                if (isset($_POST['change_pass'])){
                                    ...
                                    if ($old_pass && $has_pass){
                                        $sql = "SELECT * FROM doctors WHERE doctor_id = '$userdata[doctor_id]' AND password = '$has_pass'";
                                        $result = mysqli_query($connect, $sql);

                                        $up = mysqli_fetch_assoc($result);

                                        if ($up !=0){
                                            $change_pass = "UPDATE doctors SET password = '$new_pass_hash' WHERE doctor_id = '$userdata[doctor_id]'";
